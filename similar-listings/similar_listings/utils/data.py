...
class SessionExtractor:
    ...
    def extract_sessions(self, interactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for user_id, user_interacts in user_interactions.items():
            ...
            for interaction in user_interacts:
                ...
                if (last_timestamp is not None and 
                    timestamp - last_timestamp > self.max_session_gap):
    ...
    def _finalize_session(self, session_interactions: List[Dict[str, Any]], 
        ...
        session = {
            ...
            'session_duration': session_interactions[-1]['timestamp'] - session_interactions[0]['timestamp']
        }
...
class TripletGenerator:
    ...
    def __init__(self, window_size: Optional[int] = None, neg_sampling_ratio: Optional[int] = None, 
    ...
    def extract_sliding_window_pairs(self, session: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    def generate_negative_samples(self, central_listing: str, positive_contexts: Set[str], 
                                 all_listings: List[str], neighborhood_index: Dict[str, List[str]],
                                 listing_to_city: Dict[str, str]) -> List[str]:
        ...
        if central_city and central_city in neighborhood_index:
            ...
            while len(hard_negatives) < self.neg_sampling_ratio - len(random_negatives) and attempts < 100:
                candidate = random.choice(same_city_listings)
                if (candidate != central_listing and 
                    candidate not in positive_contexts and 
                    candidate not in negatives):
    ...
    def generate_triplets(self, sessions: List[Dict], listings: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for session_idx, session in enumerate(sessions):
            ...
            for central_listing, positive_contexts in central_to_contexts.items():
                ...
                negative_listings = self.generate_negative_samples(
                    central_listing, positive_contexts, all_listing_ids, 
                    neighborhood_index, listing_to_city
                )
                
                # Add negative triplets
                for neg_listing in negative_listings:
                    triplet_type = ('hard_negative' if listing_to_city.get(neg_listing) == 
                                   listing_to_city.get(central_listing) else 'random_negative')
