...
class RankingEvaluator:
    ...
    def __init__(self, sessions: List[Dict], listings: List[Dict], 
                 listing_to_idx: Dict[str, int], idx_to_listing: Dict[int, str]):
    ...
    def evaluate_ranking(self, embeddings: np.ndarray) -> Dict[str, float]:
        ...
        for session in self.sessions:
            ...
            clicked_indices = [self.listing_to_idx[lid] for lid in clicked_ids if lid in self.listing_to_idx]
...
def print_session_stats(sessions: List[Dict[str, Any]]) -> None:
    ...
    avg_duration = sum(session['session_duration'] for session in sessions) / total_sessions
